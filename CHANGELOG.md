...
- **`harbor db pull` can supply the remote MySQL password.** When the remote
  ...
  password from **`HARBOR_REMOTE_DB_PASSWORD`** — the same name whether you export
  it in the environment or put it in `.harbor/remote.env` (gitignored, never
  generated by Harbor) — falling back to an interactive silent prompt.
  The manifest holds only the username — never the
  password — and the password reaches the remote via `MYSQL_PWD` set *inside* the
  ssh script, so it never appears in any process list, local or remote. New
  projects gitignore `remote.env`; existing ones get the line added automatically
  whenever a `remote.env` is present. Without `user:`, behavior is unchanged
  (defer to the remote's own auth). With `user:`, the dump connects over
  `127.0.0.1` (the way the app connects) rather than the unix socket, since MySQL
  treats the socket as `@localhost` and an app user granted for `@'127.0.0.1'`/
  `@'%'` can't match it (`Access denied … @'localhost'`); override the connection
  host with `remote.db_host` (e.g. `localhost` to force the socket back).
...
- **`harbor db pull` failed with `mktemp: mkstemp failed … File exists`.** Its
  ...
  was never randomized and every pull tried to create the same literal filename,
  breaking the moment one was left behind. Create with a trailing-X template and
  rename to `.sql.gz` afterward (the extension is what `db_import` decompresses by).
- **`harbor db pull` no longer imports an empty dump when the remote `mysqldump`
  fails.** The remote ran `mysqldump | gzip` with no `pipefail`, so `gzip`'s exit
  0 masked a failed dump (access denied, wrong db name) — Harbor took a needless
  auto-backup and tried to import nothing, saved only by the truncation guard.
  Now runs `set -o pipefail` remotely so the failure propagates and the pull
  aborts before touching the local DB, with a fix hint pointing at remote creds.
