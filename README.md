...
| `harbor describe [<name>]` | Everything Harbor knows about the project, read-only: framework/docroot, URL **and whether it's linked**, the PHP version **and which source pinned it** (+ ini, shim, FPM pool), Xdebug, every service's resolved image/port/state/volume, the DB credentials, multi-store routing, and extras (node, tools, hooks, scripts, remote). Effective values, not a manifest echo. |
