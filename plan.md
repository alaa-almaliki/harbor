...
| **Multi-store** | **One mode per project** (manifest `multistore.mode: domain\|path\|none`) and **one scope** — `websites:` (`MAGE_RUN_TYPE=website`) XOR `stores:` (`store`). Domain → nginx `map` on `$http_host` + `MAGE_RUN_CODE/TYPE` + `*.<name>.test` SAN. Path → three `map`s on `$request_uri` (code, type, prefix-stripped URI) + a prefix-stripping `rewrite` + a `REQUEST_URI` override, so the prefix is Harbor's and need not equal the code; requires `web/url/use_store` **0** and `web/url/redirect_to_base` **0**. |
